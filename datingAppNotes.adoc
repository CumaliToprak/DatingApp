...
*src => app içersinde _models klasörü oluşturup Api daki Dtolarımıza denk dusecek sekilde user ve photo interfaceleri oluşturup propertyleri Dto daki propertylere göre doldururuz.*
...
Sonra user datalarını çekmek için user servisi oluştururuz.
...
export class UserService {
  ...
  getUser(id): Observable<User> {
    return this.http.get<User>(this.baseUrl + 'users/' + id, httpOptions); //get metodu option alır. bu option header alabilir. headerda da yetkilendirme için token bearer bulunabilir.
  }
}

----

apiUrl angular cli la oluşturulan environmen.ts içerisine yazdık. Böylece global olarak kullanabiliriz. Production moda geçtiğimiz zaman (projeyi publish ettiğimiz zman) environment.prod.ts e ekleriz.


*'dotnet watch run' ve 'ng serve' yapıp login olduktan sonra memberslara erişilir.*


=== Creating Member Cards to display on our Member list page


*when we create a component outside of app folder angular no longer imports its references to app.module.ts file we should add it manually.*

*for example:* src => app => members => member-card component

*Not:* Bu adımdan sonraki style ve angular adımlarını biraz atladım ta ki otomatik token gönderimine kadar. Bir sonraki başlık onu anlatıyor.


=== Using Auth0 JwtModule to send up jwt tokens automatically

Daha önce userları ve userlara ait bilgileri almak için için requestimize header ekleyip authorization tokenı da göndermiştik. Bu adımda bu tokenı manuel olarak değil otomatik olrak token göndermeyi göreceğiz.

*inside app.module.ts* import _'import { JwtModule } from '@auth0/angular-jwt';'_ and _import { HttpClientModule } from '@angular/common/http';_ => then :


*import bölümüne aşağıdaki kod satırını yazarız.*
...
Bu kod satırı daha önceden header akleyerek oluşturduğumuz token gönderme işlemini auth0 kütüphanesi sayesinde  otomatik olarak yapıyor.

uygulamaya dönüp => inspect => network yapıp => kullanıcı adı ve şifre girip login olursak => bu kullanıcıyla eşleşen kullanıcılar da listlenir.

===  Kişi detay komponenti ekleme

*memberDetail componenti oluşturup aşağıdaki gibi doldururuz*
...
export class MemberDetailComponent implements OnInit {
  ...
  loadUser() {
    this.userService.getUser(+this.route.snapshot.params['id']).subscribe( //url'deki id isimli parametreyi al.(bu id'yi routes.ts de belirlemiştik)
      (user: User) => {
        this.user = user;
      },
      (error) => {
...
*routes.ts 'e aşağıdaki satırı ekleriz.*

----
      { path: 'members/:id', component: MemberDetailComponent }, // :id => route parametresini böyle tanımlarız.
----

*member-card.component.html den kullanıcı detay butonuna basıldığında yönledirilecek route bilgilerni eklriz.*
...
*NOT:* Neil'ın en önem verdiği kısım burasıymış.

to store photos we'll use *Cloudinary*.

image::Desktop\DatingApp\course-images\img82.png[]
