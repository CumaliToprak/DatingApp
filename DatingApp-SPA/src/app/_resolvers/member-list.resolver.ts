...
export class MemberListResolver implements Resolve<User[]> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<User[]> {
    ...
    return this.userService.getUsers().pipe(   //resolve metodunu kullandığımız için subscribe olmamıza gerek yok. O otomatik olarak olur
        catchError(error => {
            this.alertify.error('Problem retrieving data');
            this.router.navigate(['/home']); //hata alınca redirect edilecek route.
